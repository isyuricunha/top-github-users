...
The `public contributions` and `private contributions` by users in France on `2023/9/9 4:20 AM UTC`. This list contains users from `France` and cities `Paris` `Marseille` `Lyon` `Bordeaux` `Toulouse` `Strasbourg` `Nice` `Nantes` `Montpellier` `Lille`.

There are `138 countries` and `674 cities` can be found [here](https://github.com/isyuricunha/top-github-users).

There are `959 users`  in France. You need at least `153 followers` to be on this list.
