...
The `public contributions` and `private contributions` by users in France on `2023/10/13 11:57 PM UTC`. This list contains users from `France` and cities `Paris` `Marseille` `Lyon` `Bordeaux` `Toulouse` `Strasbourg` `Nice` `Nantes` `Montpellier` `Lille`.

There are `138 countries` and `674 cities` can be found [here](https://github.com/isyuricunha/top-github-users).

There are `957 users`  in France. You need at least `156 followers` to be on this list.
