...
The `public contributions` and `private contributions` by users in Italy on `2024/6/26 8:09 AM UTC`. This list contains users from `Italy` and cities `Rome` `Venice` `Florence` `Turin` `Milan` `Naples` `Bologna` `Genoa`.

There are `138 countries` and `675 cities` can be found [here](https://github.com/isyuricunha/top-github-users).

There are `2082 users`  in Italy. You need at least `73 followers` to be on this list.
